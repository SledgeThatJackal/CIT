            ...
            <Table hover striped bordered variant="info" style={{borderRadius: '8px', overflow: 'hidden'}}>
                <thead>
                    {table.getHeaderGroups().map(headerGroup => {
                        return <tr key={`containerTableHeader=${headerGroup.id}`}>{headerGroup.headers.map(header => {
                            return <th key={`containerHeader-${header.id}`} colSpan={ header.colSpan }>{header.isPlaceholder ? null : flexRender(header.column.columnDef.header, header.getContext())}</th>
                        })}</tr>
                    })}
                </thead>
                <tbody>
                    {table.getRowModel().rows.map(row => {
                        return <tr key={`containerRow-${row.id}`}>{row.getVisibleCells().map(cell => {
                            return <td key={`containerCell-${cell.id}`}>{flexRender(cell.column.columnDef.cell, cell.getContext())}</td>
                        })}</tr>
                    })}
